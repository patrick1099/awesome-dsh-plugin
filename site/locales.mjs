/**
 * Locale registry — the single source of truth for internationalization.
 *
 * Adding a language:
 *   1. Add an entry here (copy an existing one, translate every string).
 *   2. Create README.<code>.md with the same category headings you declare
 *      in `categories` and one `- [name](url) - description` line per plugin.
 *   3. Add banner-<code>.png and og-<code>.png to docs/ (render from the
 *      shared card template; see scripts notes).
 * Everything else — page, hreflang set, sitemap, JSON-LD — is generated.
 *
 * The first entry is the default locale (x-default, served at /).
 */
export default [
  {
    code: 'en',
    htmlLang: 'en',
    label: 'EN',
    urlPath: '/',
    out: 'docs/index.html',
    readme: 'README.md',
    sep: '-', // list-item separator awesome-lint expects in the English file
    og: '/og-en.png',
    feed: '/feed.xml',
    feedOut: 'docs/feed.xml',
    TITLE: 'Awesome DSH Plugin — Curated DeepSeek Harness (dsh) Plugin List',
    DESC: 'A curated list of {N} DeepSeek Harness (dsh) plugins: UI, themes, sessions, memory, tools, workflow, notifications, models, development, and fun. Updated continuously.',
    SEARCH_PH: 'Search plugins…',
    CAT_TITLE: '{CAT} — Awesome DSH Plugin',
    CAT_DESC: '{N} curated {CAT} plugins for DeepSeek Harness (dsh). Install commands, descriptions, and links — updated continuously.',
    P_TITLE: '{NAME} — {CAT} · Awesome DSH Plugin',
    COPY_LABEL: 'Copy install command',
    COPY_TEXT: 'copy install',
    categories: {
      ui: 'UI Enhancements',
      usage: 'Usage & Billing',
      theme: 'Themes & Appearance',
      model: 'Models & Providers',
      session: 'Sessions & Messages',
      memory: 'Memory',
      tools: 'Tools & Capabilities',
      vision: 'Vision & Multimodal',
      skill: 'Skills',
      workflow: 'Workflow & Automation',
      notify: 'Notifications & Integrations',
      dev: 'Development & Runtime',
      market: 'Plugin Markets & Managers',
      fun: 'Just for Fun',
    },
    strings: {
      SKIP: 'Skip to plugin list',
      TAGLINE: 'A curated list of <b>DeepSeek Harness</b> (<code translate="no">dsh</code>) plugins.',
      LISTING_PRE: 'Listing',
      LISTING_POST: 'plugins',
      FULLLIST: 'Full list on GitHub ↗',
      WHATIS: 'What is DeepSeek Harness? ↗',
      NORESULT: 'No plugins match.',
      CLEAR: 'Clear filters',
      INSTALL: 'Installing',
      NPM_C: 'from npm (prebuilt)',
      TGZ_C: 'from a prebuilt release tarball',
      GH_C: 'from GitHub (first run asks for allowBuilds approval — follow the hint, retry)',
      INSTALL_NOTE: 'Any plugin you install runs third-party code with your own permissions — it can read your files, use your credentials, and reach the network, and tool approvals don’t sandbox it. GitHub-sourced plugins also run build scripts at install time. Only install sources you trust, and pin a commit (<code translate="no">github:owner/repo#sha</code>).',
      CONTRIB: 'Get your plugin listed',
      CONTRIB_BODY: 'Open a PR against <a href="https://github.com/awesome-dsh-plugin/awesome-dsh-plugin" rel="noopener">awesome-dsh-plugin</a> — one line each in every <code translate="no">README</code> language file; the site syncs automatically. Add the <a href="https://github.com/topics/dsh-plugin" rel="noopener"><code translate="no">dsh-plugin</code></a> topic to your repo too.',
      MOTTO: 'Into the Unknown',
      NOTAFF: 'Not affiliated with DeepSeek',
      DISCLAIMER: 'Plugins are developed by their respective authors; listing is not an endorsement. Installing runs third-party code — review the source and proceed at your own risk.',
      ALL: 'All',
      MARKET_LEAD: '<b>dsh-market</b> — browse and install everything on this list inside DeepSeek Harness',
      MARKET_LINK: 'About dsh-market ↗',
      NAV_SUBMIT: 'Submit',
      MARKET_GO: 'Get it',
      INSTALL_BTN: 'Install',
      MENU_MARKET: 'via dsh-market · recommended',
      MENU_MARKET_HINT: 'copy the market install command',
      MENU_CLI: 'via CLI',
      MENU_CLI_HINT: 'copy this plugin’s install command',
      COPIED: 'Copied',
      TOTOP: 'Back to top',
      CRUMB_ALL: 'All plugins',
      KICKER: 'DeepSeek Harness Plugin',
      P_INSTALL: 'Install',
      P_LINKS: 'Links',
      P_RELATED: 'More in this category',
      P_MORECAT: 'View the whole category',
      P_STARS: 'Stars',
      P_CAT: 'Category',
      P_ADDED: 'Added',
      P_GH: 'GitHub repository ↗',
      P_NPM: 'npm package ↗',
      P_MARKET: 'Install via dsh-market',
      P_README_SRC: 'Content from the project README on GitHub ↗',
    },
  },
  {
    code: 'zh',
    htmlLang: 'zh-CN',
    label: '中文',
    urlPath: '/zh/',
    out: 'docs/zh/index.html',
    readme: 'README.zh.md',
    sep: '—', // em dash: the hyphen reads as punctuation between Chinese words
    og: '/og-zh.png',
    feed: '/zh/feed.xml',
    feedOut: 'docs/zh/feed.xml',
    TITLE: 'Awesome DSH Plugin — DeepSeek Harness（dsh）插件精选列表',
    DESC: 'DeepSeek Harness（dsh）插件精选列表，收录 {N} 个：UI 增强、主题外观、会话记忆、工具、工作流、通知集成、模型接入、开发与娱乐，持续更新。',
    SEARCH_PH: '搜索插件…',
    CAT_TITLE: '{CAT} — Awesome DSH Plugin · DeepSeek Harness 插件精选',
    CAT_DESC: 'DeepSeek Harness（dsh）{CAT}插件精选，共 {N} 个：含安装命令、功能描述与仓库链接，持续更新。',
    P_TITLE: '{NAME} — {CAT} · DeepSeek Harness 插件',
    COPY_LABEL: '复制安装命令',
    COPY_TEXT: '复制安装命令',
    categories: {
      ui: 'UI 增强',
      usage: '用量与计费',
      theme: '主题与外观',
      model: '模型与账号接入',
      session: '会话与消息',
      memory: '记忆',
      tools: '工具与能力',
      vision: '视觉与多模态',
      skill: '技能包',
      workflow: '工作流与自动化',
      notify: '通知与集成',
      dev: '开发与运行时',
      market: '插件市场与管理',
      fun: '娱乐',
    },
    strings: {
      SKIP: '跳到插件列表',
      TAGLINE: '<b>DeepSeek Harness</b>（<code translate="no">dsh</code>）插件精选列表。',
      LISTING_PRE: '收录',
      LISTING_POST: '个插件',
      FULLLIST: '完整列表（GitHub）↗',
      WHATIS: '什么是 DeepSeek Harness？↗',
      NORESULT: '没有匹配的插件。',
      CLEAR: '清除筛选',
      INSTALL: '安装',
      NPM_C: 'npm 包（预构建）',
      TGZ_C: 'Release 预构建包',
      GH_C: 'GitHub 源码（首次需按提示配置 allowBuilds 构建授权后重试）',
      INSTALL_NOTE: '装任何插件都等于在你的机器上跑第三方代码，权限和你本人一样大——能读你的文件、用你的凭据、访问网络，工具审批管不到它。GitHub 来源的插件还会在安装时执行构建脚本。请只安装可信来源，并尽量锁定 commit（<code translate="no">github:owner/repo#sha</code>）。',
      CONTRIB: '收录你的插件',
      CONTRIB_BODY: '给 <a href="https://github.com/awesome-dsh-plugin/awesome-dsh-plugin" rel="noopener">awesome-dsh-plugin</a> 提一个 PR：在每个 <code translate="no">README</code> 语言文件中各加一行，网站会自动同步。也请为你的仓库打上 <a href="https://github.com/topics/dsh-plugin" rel="noopener"><code translate="no">dsh-plugin</code></a> topic。',
      MOTTO: '探索未至之境',
      NOTAFF: '与 DeepSeek 无隶属关系',
      DISCLAIMER: '插件由各自作者开发维护，收录不构成背书；安装即运行第三方代码，请自行审阅源码、风险自担。',
      ALL: '全部',
      MARKET_LEAD: '<b>dsh-market</b> —— 在 DeepSeek Harness 里直接逛并安装本列表全部插件',
      MARKET_LINK: '了解 dsh-market ↗',
      NAV_SUBMIT: '提交收录',
      MARKET_GO: '获取',
      INSTALL_BTN: '安装',
      MENU_MARKET: '通过 dsh-market · 推荐',
      MENU_MARKET_HINT: '复制市场安装命令',
      MENU_CLI: '通过命令行',
      MENU_CLI_HINT: '复制本插件的安装命令',
      COPIED: '已复制',
      TOTOP: '返回顶部',
      CRUMB_ALL: '全部插件',
      KICKER: 'DeepSeek Harness 插件',
      P_INSTALL: '安装',
      P_LINKS: '链接',
      P_RELATED: '同类插件',
      P_MORECAT: '查看整个分类',
      P_STARS: 'Star 数',
      P_CAT: '分类',
      P_ADDED: '收录于',
      P_GH: 'GitHub 仓库 ↗',
      P_NPM: 'npm 包 ↗',
      P_MARKET: '通过 dsh-market 安装',
      P_README_SRC: '内容来自项目 README（GitHub）↗',
    },
  },
  {
    code: 'ja',
    htmlLang: 'ja',
    label: '日本語',
    urlPath: '/ja/',
    out: 'docs/ja/index.html',
    readme: 'README.ja.md',
    sep: '—', // em dash: a hyphen reads as punctuation between Japanese words
    og: '/og-en.png', // TODO: swap for /og-ja.png once the card is rendered
    feed: '/ja/feed.xml',
    feedOut: 'docs/ja/feed.xml',
    TITLE: 'Awesome DSH Plugin — DeepSeek Harness（dsh）プラグイン厳選リスト',
    DESC: 'DeepSeek Harness（dsh）プラグインの厳選リスト、{N} 件を収録：UI 拡張、テーマと外観、セッションとメモリ、ツール、ワークフロー、通知と連携、モデル接続、開発とお楽しみ。継続的に更新中。',
    SEARCH_PH: 'プラグインを検索…',
    CAT_TITLE: '{CAT} — Awesome DSH Plugin · DeepSeek Harness プラグイン厳選リスト',
    CAT_DESC: 'DeepSeek Harness（dsh）の{CAT}プラグイン厳選 {N} 件。インストールコマンド、説明、リポジトリへのリンク付きで継続的に更新中。',
    P_TITLE: '{NAME} — {CAT} · DeepSeek Harness プラグイン',
    COPY_LABEL: 'インストールコマンドをコピー',
    COPY_TEXT: 'コマンドをコピー',
    categories: {
      ui: 'UI 拡張',
      usage: '使用量と課金',
      theme: 'テーマと外観',
      model: 'モデルとプロバイダー',
      session: 'セッションとメッセージ',
      memory: 'メモリ',
      tools: 'ツールと機能',
      vision: 'ビジョンとマルチモーダル',
      skill: 'スキル',
      workflow: 'ワークフローと自動化',
      notify: '通知と連携',
      dev: '開発とランタイム',
      market: 'プラグインマーケットと管理',
      fun: 'お楽しみ',
    },
    strings: {
      SKIP: 'プラグイン一覧へスキップ',
      TAGLINE: '<b>DeepSeek Harness</b>（<code translate="no">dsh</code>）プラグインの厳選リスト。',
      LISTING_PRE: '全',
      LISTING_POST: '件を掲載',
      FULLLIST: '完全なリスト（GitHub）↗',
      WHATIS: 'DeepSeek Harness とは？↗',
      NORESULT: '一致するプラグインはありません。',
      CLEAR: '絞り込みを解除',
      INSTALL: 'インストール',
      NPM_C: 'npm パッケージ（ビルド済み）',
      TGZ_C: 'Release のビルド済み tarball',
      GH_C: 'GitHub ソース（初回は allowBuilds の承認を求められます。表示される案内に従って再実行してください）',
      INSTALL_NOTE: 'プラグインをインストールすると、あなたと同じ権限で第三者のコードが動きます——ファイルを読み、認証情報を使い、ネットワークにも接続でき、ツール承認では隔離できません。GitHub 由来のプラグインはインストール時にビルドスクリプトも実行します。信頼できる提供元だけをインストールし、できればコミットを固定してください（<code translate="no">github:owner/repo#sha</code>）。',
      CONTRIB: 'あなたのプラグインを掲載する',
      CONTRIB_BODY: '<a href="https://github.com/awesome-dsh-plugin/awesome-dsh-plugin" rel="noopener">awesome-dsh-plugin</a> に PR を送ってください——各言語の <code translate="no">README</code> に 1 行ずつ追加すれば、サイトは自動で同期されます。リポジトリに <a href="https://github.com/topics/dsh-plugin" rel="noopener"><code translate="no">dsh-plugin</code></a> topic を付けるのもお忘れなく。',
      MOTTO: '未知の世界へ',
      NOTAFF: 'DeepSeek とは無関係です',
      DISCLAIMER: 'プラグインは各作者が開発しており、掲載は推奨を意味しません。インストールは第三者のコードの実行を伴います——ソースを確認のうえ、自己責任でご利用ください。',
      ALL: 'すべて',
      MARKET_LEAD: '<b>dsh-market</b> —— DeepSeek Harness の中でこのリストのプラグインをそのまま閲覧・インストール',
      MARKET_LINK: 'dsh-market について ↗',
      NAV_SUBMIT: '掲載を申請',
      MARKET_GO: '入手',
      INSTALL_BTN: 'インストール',
      MENU_MARKET: 'dsh-market から · 推奨',
      MENU_MARKET_HINT: 'マーケットのインストールコマンドをコピー',
      MENU_CLI: 'コマンドラインから',
      MENU_CLI_HINT: 'このプラグインのインストールコマンドをコピー',
      COPIED: 'コピーしました',
      TOTOP: 'トップへ戻る',
      CRUMB_ALL: 'すべてのプラグイン',
      KICKER: 'DeepSeek Harness プラグイン',
      P_INSTALL: 'インストール',
      P_LINKS: 'リンク',
      P_RELATED: '同じカテゴリのプラグイン',
      P_MORECAT: 'カテゴリ全体を見る',
      P_STARS: 'Star 数',
      P_CAT: 'カテゴリ',
      P_ADDED: '掲載日',
      P_GH: 'GitHub リポジトリ ↗',
      P_NPM: 'npm パッケージ ↗',
      P_MARKET: 'dsh-market からインストール',
      P_README_SRC: '内容はプロジェクトの README（GitHub）より ↗',
    },
  },
]
